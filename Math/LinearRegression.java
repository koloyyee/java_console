...
public class LinearRegression{
    ...
    public static void main(String... args) {
        ...
        double[] sqPredErr = eq.sqPredictionErrors(x, pred);
    ...
    public double slope(double[] x, double[] y) {
        ...
        m = (x.length * xySum - (xSum * ySum)) / (x.length * xSqSum - sqXSum);
        return m;
    }
    ...
    public double[] sqPredictionErrors(double[] x, double[] predictions) {
        return IntStream.range(0, x.length)
            .mapToDouble(i -> Math.pow(x[i] - predictions[i], 2))
            .toArray();
    }
}
